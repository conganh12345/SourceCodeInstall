...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form action="{{ route('all_delete_post') }}" method="POST">
            @csrf
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    ...
                    <a href="{{route('all_add_post')}}" class="btn btn-success">Tạo mới</a>
                    <!-- Button to delete all posts (you may replace '#delete-all' with the actual route) -->
                    <a href="{{route('all_delete_allpost')}}" class="btn btn-danger" id="delete-all">Xóa tất cả</a>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>
                                ...
                                <tbody>
                                    ...
                                    @foreach($posts as $post)
                                        <tr>
                                            ...
                                            <td>{{ $post->status }}  <!-- Box select để thay đổi trạng thái -->
                                                 </td>
                                            ...
                                            <td>
                                                <!-- Icon show -->
                                   <!-- Button show -->
                                                    <button onclick="window.location='{{ route('all_show_post', $post->id) }}';" class="btn btn-info btn-sm" style="margin-right: 10px;"><i class="fas fa-eye"></i></button>

                                                    <!-- Button edit -->
                                                    <button onclick="window.location='{{ route('all_edit_post', $post->id) }}';" class="btn btn-warning btn-sm" style="margin-right: 10px;"><i class="fas fa-edit"></i></button>
